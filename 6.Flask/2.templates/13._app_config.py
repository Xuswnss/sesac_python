...
app.config['ALLOWED_FILE_EXT'] = {'png', 'jpg'}
app.config['MAX_CONTENT_LENGTH'] = 1 * 1024 * 1024

os.makedirs(app.config['UPLOAD_FOLDER'] , exist_ok= True ) # 폴더 없으면 폴더 만들어주기
@app.route('/')
def index():
...
def too_large(e):
    size_mb = app.config['MAX_CONTENT_LENGTH']   / (1024^2)
    return f'업로드한 파일이 너무 큽니다. 최대 {size_mb}MB 까지만 허용!!'
